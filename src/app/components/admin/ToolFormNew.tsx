    ...
    <div className="space-y-6 p-6 bg-white/5 rounded-lg">
      ...
      <div>
        ...
        <Textarea
          ...
          value={(version.features || []).join('\n')}
          onChange={(e) =>
            onUpdate({ features: e.target.value.split('\n') })
          }
          className={`bg-black/50 border-white/20 text-white transition-all duration-700 ${vhl('features')}`}
          rows={4}
        />
        <p className="text-xs text-white/40 mt-1">One feature per line</p>
      </div>
